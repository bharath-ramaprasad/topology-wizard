...
WireIt.Layer = function(options) {
   ...
   /** 
    * Node - Network Element Properties
   ...
   /**
    * Event that is re-fired when a terminal is selected to do some net element proeperties processing
    * * @param {} options
    * @returns {} 
    */
     this.eventTerminalSelectionReFire = new YAHOO.util.CustomEvent("eventTerminalSelectionReFire");
   
   
    /**
    * Event that is fired when a property of the network element property value is changed
    * You can register this event with container.eventChangedNodeProps.subscribe(function(e,params) { var wire=params[0];}, scope);
    * @event eventChangedNetElemProps
    */
    this.eventChangedNodeProps = new YAHOO.util.CustomEvent("eventChangedNodeProps");
   
   /**
    * Event that is fired when a property of the network element property value is changed
    * You can register this event with container.eventNodeSelection.subscribe(function(e,params) { var wire=params[0];}, scope);
    * @event eventChangedNetElemProps    */
    
    this.eventNodeSelection = new YAHOO.util.CustomEvent("eventNodeSelection");
    
    /**
    * Event that is fired when a property of the network element property is removed 
    * You can register this event with container.eventRemovePropertiesPanel.subscribe(function(e,params) { var wire=params[0];}, scope);
     * @event eventRemovePropertiesPanel 
     */
     
    this.eventRemovePropertiesPanel = new YAHOO.util.CustomEvent("eventRemovePropertiesPanel"); 
   
    this.currentSelectedContainer = null;
    
    this.prevSelectedNetEl = null;
    
    this.nelProperties = null; 
...
WireIt.Layer.prototype = {
   ...
   */
   ...
   getNetELType: function(netElObj) { 

		if((netElObj instanceof eval("WireIt.Terminal")))
		{
			return "Terminal";
			
		}
		else if((netElObj instanceof eval("WireIt.Container")))
		{
			return "Container";
		}
		else if((netElObj instanceof eval("WireIt.Wire")))
		{
			return "Connector";
		}
   ...
   addWire: function(wireConfig) {
      ...
      //added to restrict topowiz from drawing more than one wire from or to a terminal 
      if(terminal1.wires.length == 0 && terminal2.wires.length == 0 )
      {
     	 var wire = new type( terminal1, terminal2, this.el, wireConfig);
      
     	 if(wireConfig.linkProps != null || !(YAHOO.lang.isUndefined(wireConfig.linkProps)))
      	 {
   		 	  wire.setValue(wireConfig.linkProps);
      	 }
   ...
   /**
    * Instantiate a container given its "xtype": WireIt.Container (default) or a subclass of it.
    * @method addContainer
   ...
   addContainer: function(containerConfig) {
     ...
     // alert('type  = ' +  type );
	  ...
	  var obj_cont_info =new Array();
	  obj_cont_info[0] = container;
	  obj_cont_info[1] = containerConfig; 
	
  	  YAHOO.util.Event.addListener(container.bodyEl,"click", this.onContainerClick, obj_cont_info, this);
      ...
      //@B@S
      container.eventChangedNetElemProps.subscribe(this.onEventNetElemChanged, this, true);
      container.eventTerminalSelection.subscribe(this.onEventTerminalSelection, this, true);
   	  //@B@E
   ...
   removePropertiesPanel: function(container)
   {
   		var properties_panel = YAHOO.util.Dom.get('nelem');
   		
   		if(this.hasChildElems(properties_panel))
		{
				while ( properties_panel.childNodes.length >= 1 )
			    {
					properties_panel.removeChild( properties_panel.firstChild );       
				}
			
			  this.eventRemovePropertiesPanel.fire(this);
  		} 
   },
    ...
    */
	...
	onContainerClick: function(e ,obj_cont_info) 
	{
		
		 if(obj_cont_info[0].layer) 
		 {
            if(obj_cont_info[0].layer.focusedContainer && obj_cont_info[0].layer.focusedContainer != obj_cont_info[0]) 
            {
            	obj_cont_info[0].layer.focusedContainer.removeFocus();
        	}
         	obj_cont_info[0].setFocus();
         	
         	obj_cont_info[0].layer.focusedContainer = obj_cont_info[0];
      	}
		...
		{	
			   	...
			   	var typeof_netel  = this.getNetELType(this.prevSelectedNetEl); 
				...
				if(typeof_netel == "Terminal")
				{
					...
					this.prevSelectedNetEl.setDropInvitation(false);
					
					updated_values = this.prevSelectedNetEl.neleProperties.getValue();
					...
					this.prevSelectedNetEl.portID = updated_values["portID"];
					this.prevSelectedNetEl.Capacity = updated_values["capacity"];
					this.prevSelectedNetEl.maxResCapacity = updated_values["maxResCapacity"];
					this.prevSelectedNetEl.minResCapacity = updated_values["minResCapacity"];
					this.prevSelectedNetEl.granularity = updated_values["granularity"];
					
				}
				else if (typeof_netel == "Container")
				{
					updated_values = this.nelProperties.getValue();
					...
					if(updated_values["domainid"] == "" || updated_values["nodeid"] == "" || updated_values["nodeaddress"] == "") 
					{
				 	...
				 	this.prevSelectedNetEl.domainid = updated_values["domainid"];
				 	this.prevSelectedNetEl.nodeid = updated_values["nodeid"];
				 	this.prevSelectedNetEl.nodeaddress = updated_values["nodeaddress"];	
				}
				else if(typeof_netel == "Connector")
				{
					updated_values = this.prevSelectedNetEl.nelProperties.getValue();
					...
					if(updated_values["linkid"] == "" || updated_values["remoteLinkId"] == "" || updated_values["trafficEngineeringMetric"] == "" || updated_values["encodingType"] == "" || updated_values["interfaceMTU"] == "" || updated_values["vlanRangeAvailability"] == "" ) 
					{
				 	...
				 	this.prevSelectedNetEl.linkid = updated_values["linkid"];
				 	this.prevSelectedNetEl.remoteLinkId = updated_values["remoteLinkId"];
				 	this.prevSelectedNetEl.trafficEngineeringMetric = updated_values["trafficEngineeringMetric"];
				 	this.prevSelectedNetEl.switchingcapType = updated_values["switchingcapType"];
				 	this.prevSelectedNetEl.encodingType = updated_values["encodingType"];
				 	this.prevSelectedNetEl.capability = updated_values["capability"];
				 	this.prevSelectedNetEl.interfaceMTU = updated_values["interfaceMTU"];
				 	this.prevSelectedNetEl.vlanRangeAvailability = updated_values["vlanRangeAvailability"];			 		
		...
		parent_box.appendChild(WireIt.cn('span', null, null, obj_cont_info[1].name + " Properties:"));
		
				
			this.nelpropertiesFields =  [
			
			{"type": "string", inputParams: {"name": "domainid", "id": "domainid",label: "Domain ID", value: obj_cont_info[0].domainid } },
			{"type": "string", inputParams: {"name": "nodeid", "id": "nodeid", label: "Node ID", value: obj_cont_info[0].nodeid }},
			{"type": "string", inputParams: {"name": "nodeaddress", "id": "nodeaddr", label: "Node Address", value: obj_cont_info[0].nodeaddress }}
			
	 		];
			
						

		this.nelProperties = new inputEx.Group({
 		parentEl: YAHOO.util.Dom.get('nelem'),
	    fields: this.nelpropertiesFields
		});

			
		this.eventNodeSelection.fire(this);
			
		
		
		this.nelProperties.updatedEvt.subscribe(function() {
				this.eventChanged.fire(this);
				}, this, true);
		
		
				
		obj_cont_info[0].nelProperties = this.nelProperties;
		
		this.prevSelectedNetEl = obj_cont_info[0];
						 									
	    // NMWG_Domain_Prefix	
	
	},
	
	addContainerTerminal : function()
	{
			var typeof_netel  = this.getNetELType(this.prevSelectedNetEl);
			
			if(typeof_netel == "Container")
			{
				//only if container then add a terminal to this container
				this.prevSelectedNetEl.addPort();
								
			}
				
	},
	deleteContainerTerminal : function()
	{
			var typeof_netel  = this.getNetELType(this.prevSelectedNetEl);
			
			if(typeof_netel == "Terminal")
			{
				//only if container then delete a terminal to this container
				this.prevSelectedNetEl.removePort();						
			}
...
{ 
	...
	onEventNetElemChanged: function(){
	...
	this.eventTerminalSelectionReFire.fire(this);
   ...
   removeContainer: function(container) {
      ...
      if( index != -1 ) {
         container.remove();
         this.removePropertiesPanel(container);
   ...
   onAddWire: function(event, args) {
      ...
      var obj_connect_info =new Array();
	  obj_connect_info[0] = wire;
	 // obj_connect_info[1] = wireConfig; 
         
         if(this.options.enableMouseEvents) {
            YAHOO.util.Event.addListener(wire.element, "mousemove", this.onWireMouseMove, this, true);
            YAHOO.util.Event.addListener(wire.element, "click", this.onConnectorClick, obj_connect_info, this);
    ...
    onConnectorClick: function(e ,obj_connect_info)
    {
		...
		{	
				...
				//alert("typeof netel from layer: " + typeof_netel);
				...
				else if (typeof_netel == "Connector")
				{
					...
					if(updated_values["linkid"] == "" || updated_values["remoteLinkId"] == "" || updated_values["trafficEngineeringMetric"] == "" || updated_values["encodingType"] == "" || updated_values["interfaceMTU"] == "" || updated_values["vlanRangeAvailability"] == "" ) 
					{
						alert("property values for the previously selected connector is null. Please define all the properties");
						return;
					}
		...
		parent_box.appendChild(WireIt.cn('span', null, null, obj_connect_info[0].name + " Properties:"));
		
		var link_info_box = WireIt.cn('div', {className: "Nel-Link-Info"});
		
		//alert( "src: " + obj_connect_info[0].terminal1.container.domainid + ":" + obj_connect_info[0].terminal1.container.nodeid + "<---> dest: " + obj_connect_info[0].terminal2.container.domainid + ":" + obj_connect_info[0].terminal2.container.nodeid );
		
		link_info_box.appendChild(WireIt.cn('span', null, null, " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Node ID  ==> " + obj_connect_info[0].terminal1.container.domainid + ":" + obj_connect_info[0].terminal1.container.nodeid));
		
		link_info_box.appendChild(WireIt.cn('br', null, null));
		
		
		link_info_box.appendChild(WireIt.cn('span', null, null, " Remote Node ID ==> " + obj_connect_info[0].terminal2.container.domainid + ":" + obj_connect_info[0].terminal2.container.nodeid));
		
		link_info_box.appendChild(WireIt.cn('br', null, null));
		link_info_box.appendChild(WireIt.cn('br', null, null));
		
		nel_properties.appendChild(link_info_box);
				
			this.nelpropertiesFields =  [
			
			{"type": "string", inputParams: {"name": "linkid", "id": "linkid",label: "Link ID", value: obj_connect_info[0].linkid } },
			{"type": "string", inputParams: {"name": "remoteLinkId", "id": "remoteLinkId", label: "Remote Link ID", value: obj_connect_info[0].remoteLinkId }},
			{"type": "string", inputParams: {"name": "trafficEngineeringMetric", "id": "trafficEngineeringMetric", label: "Traffic Engg Metric", value: obj_connect_info[0].trafficEngineeringMetric }},
			{"type": "string", inputParams: {"name": "switchingcapType", "id": "switchingcapType", label: "Switching Cap Type",typeInvite: "optional", value: obj_connect_info[0].switchingcapType }},
			{"type": "string", inputParams: {"name": "encodingType", "id": "encodingType", label: "Encoding Type", value: obj_connect_info[0].encodingType }},
			{"type": "string", inputParams: {"name": "capability", "id": "capability", label: "Capability", typeInvite: "optional", value: obj_connect_info[0].capability }},
			{"type": "string", inputParams: {"name": "interfaceMTU", "id": "interfaceMTU", label: "Interface MTU", value: obj_connect_info[0].interfaceMTU }},
			{"type": "string", inputParams: {"name": "vlanRangeAvailability", "id": "vlanRangeAvailability", label: "VLAN Range", value: obj_connect_info[0].vlanRangeAvailability }}
	 		];
		...
		//YAHOO.util.Dom.setStyle(this.nelProperties, 'height', '100%');
		//YAHOO.util.Dom.setStyle(this.nelProperties, 'overflow', 'scroll');
		...
		obj_connect_info[0].nelProperties = this.nelProperties;
		
		this.prevSelectedNetEl = obj_connect_info[0];
   ...
   onRemoveWire: function(event, args) {
      ...
      if( index != -1 ) {
      	 this.removePropertiesPanel(wire);
